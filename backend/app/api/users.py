...
from app.models.models import UserLearningProgress, SavedLearningPath
...
async def update_progress(progress: UserProgressUpdate, user_id: int = Depends(require_auth), db: Session = Depends(get_db)):
    existing = db.query(UserLearningProgress).filter(UserLearningProgress.user_id == user_id, UserLearningProgress.technology_slug == progress.technology_slug).first()
    if existing:
    ...
    record = UserLearningProgress(user_id=user_id, technology_slug=progress.technology_slug, status=progress.status, score=progress.score,
...
async def save_path(title: str, goal: str, path_data: dict, user_id: int = Depends(require_auth), db: Session = Depends(get_db)):
...
async def delete_saved_path(path_id: int, user_id: int = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    db.delete(path); db.commit(); return {"message": "Path deleted"}


@router.get("/users/audit-logs", response_model=list[AuditLogResponse])
async def get_audit_logs(skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=200), user_id: int = Depends(require_auth), db: Session = Depends(get_db)):
